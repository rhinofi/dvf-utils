...
describe('Joi', () => {
  ...
  describe('should return error if the value has more than the allowed decimal spaces', () => {
    it('price', () => {
      const schema = Joi.price().required()
      expect(schema.validate('250.12345678').error.message)
        .toEqual('"value" must not have more than 5 decimal spaces')
    })
    it('amount', () => {
      const schema = Joi.amount().required()
      expect(schema.validate('0.1234567899').error.message)
        .toEqual('"value" must not have more than 8 decimal spaces')
    })
  })
})
