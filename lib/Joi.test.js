...
describe('Joi', () => {
  ...
  it('starkNetAddress', () => {
    const schema = Joi.starkNetAddress().required()

    // Set expected number of assertion to make sure that asserts wrapped in
    // testErrorMessage helper are actually executed.
    expect.assertions(2 + 2 + 4 + 6)
    const validAddresses = [
      '0x001849addcd238a1a955fd218cf26b6666ced1078a126ef820da88ffe0d11baa',
      '0x001849ADDCD238A1A955FD218CF26B6666CED1078A126EF820DA88FFE0D11BAA',
    ]
    ...
    invalidValues.forEach(testError('"value" must be a string'))

    const invalidStrings = [
      'abc',
      // Valid eth address but we want StarkNet address
      '0xCc6E2d20cC5AaFDCa329bA2d63e5ba5edD684B2F',
      '0xCc6E2d20cC5AaFDCa329bA2d63e5ba5edD684B2f',
      '2d5752b42440139a130f453e0c646e6e656dded4',
      // First zero missing. Might be valid but we want to force 64 char format.
      '0x01849addcd238a1a955fd218cf26b6666ced1078a126ef820da88ffe0d11baa',
      // Extra zero
      '0x0001849addcd238a1a955fd218cf26b6666ced1078a126ef820da88ffe0d11baa',
    ]

    invalidStrings.forEach(
      testError('"value" must be a valid StarkNet Address'),
    )
  })
