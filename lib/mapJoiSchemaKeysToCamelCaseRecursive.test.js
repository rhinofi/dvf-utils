...
const mapJoiSchemaKeysToCamelCaseRecursive = require('./mapJoiSchemaKeysToCamelCaseRecursive')
...
describe('mapJoiSchemaKeysToCamelCaseRecursive', () => {
  it('maps Joi schema keys to camelCase (recursively)', () => {
    ...
    expect(getDescryptions(mapJoiSchemaKeysToCamelCaseRecursive([schemaSnakeCase, schemaSnakeCase])))
      .toEqual(getDescryptions([schemaCamelCase, schemaCamelCase]))

    // Handles multiple schemas in an object (maps object keys)
    expect(getDescryptions(mapJoiSchemaKeysToCamelCaseRecursive({ s_1: schemaSnakeCase, s_2: schemaSnakeCase })))
