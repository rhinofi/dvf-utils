...
const isStarkNetAddress = require('./isStarkNetAddress')
...
module.exports = Joi
  ...
  .extend(extendedJoi => ({
    ...
    messages: {
      'starkNetAddress.base': '{{#label}} must be a valid StarkNet Address',
    },
    validate: (value, helpers) => {
      if (!isStarkNetAddress(value)) {
        return { value, errors: helpers.error('starkNetAddress.base') }
      }
    },
  }))
  .extend(extendedJoi => ({
