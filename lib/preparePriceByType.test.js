...
describe('preparePriceByType', () => {
  ...
  it('accepts buy/ask and sell/bid', () => {
    ...
    expect(preparePriceByType(price, 'ask')).toBe(
      preparePriceByType(price, 'buy'),
    )
    expect(preparePriceByType(price, 'aSk')).toBe(
      preparePriceByType(price, 'BuY'),
    )
    expect(preparePriceByType(price, 'bid')).toBe(
      preparePriceByType(price, 'sell'),
    )
    expect(preparePriceByType(price, 'BiD')).toBe(
      preparePriceByType(price, 'sElL'),
    )
    ...
    expect(preparePriceByType(price, 'ask')).not.toBe(
      preparePriceByType(price, 'sell'),
    )
    expect(preparePriceByType(price, 'aSk')).not.toBe(
      preparePriceByType(price, 'sElL'),
    )
    expect(preparePriceByType(price, 'bid')).not.toBe(
      preparePriceByType(price, 'buy'),
    )
    expect(preparePriceByType(price, 'BiD')).not.toBe(
      preparePriceByType(price, 'BuY'),
